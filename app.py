...
# Define temp directory as a constant
TEMP_DIR = Path("./temp")
...
class GenerateMusicRequest(BaseModel):
    ...
    repo_id: str = "ASLP-lab/DiffRhythm-base"  # Added repo_id parameter
...
# Function to load models - Updated to match new signature
def load_models(max_frames=2048, repo_id="ASLP-lab/DiffRhythm-base"):
    global device
    ...
    print(f"Loading models from infer_utils.prepare_model(max_frames={max_frames}, device={device}, repo_id={repo_id})...")
...
async def process_music_generation(lyrics_text, ref_audio_path=None, ref_prompt=None, output_path=None, params=None):
    ...
    # Updated to match new get_lrc_token signature
...
async def generate_music(
    ...
    repo_id: str = Form("ASLP-lab/DiffRhythm-base")
):
    """
    ...
    - **repo_id**: Model repository ID (default: "ASLP-lab/DiffRhythm-base")
    """
    if models is None:
        raise HTTPException(
            status_code=503, 
...
# Load models on startup with default parameters
...
async def startup_event():
    ...
    # Default frame size for 95 seconds
    max_frames = 2048
    
    print("Loading models...")
    models = load_models(max_frames=max_frames)
